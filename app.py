...
def login():
    ...
    email = ''
    if request.method == 'POST':
        # Step 1: Email/password form submission
        ...
        # Step 2: OTP form submission
            ...
            email = session.get('login_user_email', '')
            if otp == session.get('login_otp'):
                # Successful login
                session['user_id'] = session.get('login_user_id')
                session['user_name'] = session.get('login_user_name')
                session['user_email'] = session.get('login_user_email')
        ...
        # GET request: clear any previous login OTP session
        session.pop('login_otp', None)
        session.pop('login_user_id', None)
        session.pop('login_user_name', None)
        session.pop('login_user_email', None)
    return render_template('login.html', message=message, otp_sent='login_otp' in session, email=email)
